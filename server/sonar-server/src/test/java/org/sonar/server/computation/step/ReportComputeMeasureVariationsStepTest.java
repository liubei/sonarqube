...
public class ReportComputeMeasureVariationsStepTest {
  ...
  @Test
  public void set_variation_to_raw_value_when_component_was_created_after_period() throws Exception {
    // Project
    SnapshotDto period1ProjectSnapshot = newSnapshotForProject(PROJECT_DTO).setCreatedAt(1000_000_000L);
    SnapshotDto afterPeriod1ProjectSnapshot = newSnapshotForProject(PROJECT_DTO).setCreatedAt(2000_000_000L);
    dbClient.snapshotDao().insert(session, period1ProjectSnapshot);
    dbClient.snapshotDao().insert(session, afterPeriod1ProjectSnapshot);
    dbClient.measureDao().insert(session, newMeasureDto(ISSUES_METRIC.getId(), PROJECT_DTO.getId(), period1ProjectSnapshot.getId(), 60d));
    dbClient.measureDao().insert(session, newMeasureDto(ISSUES_METRIC.getId(), PROJECT_DTO.getId(), afterPeriod1ProjectSnapshot.getId(), 60d));

    // Directory
    ComponentDto directoryDto = ComponentTesting.newDirectory(PROJECT_DTO, "dir");
    dbClient.componentDao().insert(session, directoryDto);
    SnapshotDto afterPeriod1DirectorySnapshot = createForComponent(directoryDto, afterPeriod1ProjectSnapshot).setCreatedAt(2000_000_000L);
    dbClient.snapshotDao().insert(session, afterPeriod1DirectorySnapshot);
    dbClient.measureDao().insert(session, newMeasureDto(ISSUES_METRIC.getId(), directoryDto.getId(), afterPeriod1DirectorySnapshot.getId(), 10d));
    session.commit();
